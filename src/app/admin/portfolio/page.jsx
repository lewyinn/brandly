...
export default function DashboardPage() {
    ...
    const [sortField, setSortField] = useState('judul');
    ...
    async function load(q = '') {
        setLoading(true);
        const url = q ? `/api/portfolios?q=${encodeURIComponent(q)}` : '/api/portfolios';
    ...
    const filteredData = useMemo(() => {
        ...
        return items.filter(p =>
            ...
            (p.link_portfolio || '').toLowerCase().includes(s) ||
                    ...
                    <div className="flex items-center space-x-4">
                        ...
                        <div>
                            <nav className="text-sm text-gray-500 dark:text-gray-400">
                                ...
                                <span className="text-gray-900 dark:text-gray-200 font-medium">Portfolio</span>
                            </nav>
                        </div>
                    </div>
                ...
                <div className="flex flex-col md:flex-row sm:justify-between mb-6">
                    <div className="flex flex-col md:flex-col-reverse justify-between items-start w-full sm:w-auto gap-0 md:gap-2 mb-4 md:mb-0">
                        <div className="flex flex-col gap-0 md:gap-1 mb-2">
                            <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-4 sm:mb-0">Portfolio Management</h1>
                            <p className="text-sm text-gray-500 dark:text-gray-400">Kelola hasil pekerjaan: Website & Social Media</p>
                        </div>
                        <Link href={'/admin/portfolio/create'} className="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm overflow-hidden transition-colors">
                    <div className="hidden lg:block overflow-x-auto">
                        <table className="w-full">
                            <thead className="bg-gray-50 dark:bg-gray-700">
                                <tr>
                                    <th onClick={() => handleSort('judul')} className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600">
                                        <div className="flex items-center space-x-1">
                                            <span>Judul</span>
                                            {sortField === 'judul' && (sortDirection === 'asc' ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />)}
                                        </div>
                                    </th>
                                    ...
                                    <th onClick={() => handleSort('link_portfolio')} className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600">
                                        <div className="flex items-center space-x-1">
                                            <span>Link</span>
                                            {sortField === 'link_portfolio' && (sortDirection === 'asc' ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />)}
                                        </div>
                                    </th>
                            ...
                            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                {loading ? (
                                    ...
                                    <tr><td colSpan={6} className="px-6 py-8 text-center text-gray-500 dark:text-gray-400">No portfolio found</td></tr>
                                ) : paginatedData.map((p) => (
                                    <tr key={p.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="text-sm font-medium text-gray-900 dark:text-white">{p.judul}</div>
                                            {p.deskripsi && <div className="text-xs text-gray-500 dark:text-gray-400 line-clamp-1 w-auto">{p.deskripsi}</div>}
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <td className="px-6 py-4">
                                            <FeatureBadges features={p.tech} />
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                                            {p.link_portfolio ? (
                                                <a href={p.link_portfolio} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline break-all">
                                                    {p.link_portfolio}
                                                </a>
                                            ) : (
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div className="flex items-center space-x-2 justify-end">
                                                <button className="text-blue-600 hover:text-blue-700 p-1 cursor-pointer" title="View" onClick={() => window.open(p.link_portfolio || '#', '_blank')}>
                                                    <Eye className="w-4 h-4" />
                                                </button>
                                                <Link href={`/admin/portfolio/${p.id}/edit`} className="text-green-600 hover:text-green-700 p-1 cursor-pointer" title="Edit">
                    ...
                    <div className="lg:hidden divide-y divide-gray-200 dark:divide-gray-700">
                        {loading ? (
                            ...
                            <div className="p-6 text-center text-gray-500 dark:text-gray-400">No portfolio found</div>
                        ) : (
                            paginatedData.map((p) => (
                                <div key={p.id} className="p-4">
                                    {/* Header: Judul + Type badge */}
                                    ...
                                    <div className="mt-3 grid grid-cols-2 gap-3">
                                        <div className="col-span-2">
                                            ...
                                            {p.link_portfolio ? (
                                                <a href={p.link_portfolio} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-600 hover:underline break-all">
                                                    {p.link_portfolio}
                                                </a>
                                            ) : (
                                    ...
                                    {/* Tech */}
                                    <div className="mt-3">
                                        <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Tech</p>
                                        <FeatureBadges features={p.tech} />
                                    </div>
                                    ...
                                    <div className="mt-4 flex items-center justify-end gap-3">
                                        <button
                                            ...
                                            title="View"
                                            onClick={() => window.open(p.link_portfolio || '#', '_blank')}
                                        >
                                            <Eye className="w-4 h-4" />
                                            <span>View</span>
                                        </button>
